                ...
                try {
                    ...
                    if ($res) {
                        foreach ($res as $re) {  #  遍历每个 视频 id
                            MonitorVideoModel::create()->where(['id' => $re['id']])->update(['updated' => time(), 'status' => 6]);   #改成  6 正在使用中
                            //随机获取 cookie
                                            ...
                                            if ($json_data['comments'] && count($json_data['comments']) > 0) {
                                                foreach ($json_data['comments'] as $comment) {
                                                    try {
                                                        ...
                                                        var_dump("视频id:" . $re['vID'] . " 采集到粉丝:" . $comment['user']['uid']);
                                                        //插入粉丝
                                                        $add = [
...
//                                                $add['sex'] = $this->img_url_to_base64($comment['user']['avatar_thumb']['url_list'][0]);
                    ...
                    var_dump("进程 AutomaticFanCollectionProcess 异常:" . $exception->getMessage());
                }
            }
        });
    }
