...
class ShoppingProcess extends AbstractProcess
{
        ...
        go(function () {
            while (true) {
                \EasySwoole\RedisPool\RedisPool::invoke(function (\EasySwoole\Redis\Redis $redis) {
                    $data = $redis->rPop("Shopping");#宠物id  时间戳 价格
                    if (!$data) {
                        return false;
                    }
                    ...
                    DbManager::getInstance()->invoke(function ($client) use ($data_array, $redis) {
                        $result = $this->GetCwFormId($data_array[0]);
                        if (!$result) {
                            # 查询宠物失败
                            $redis->hSet($data_array[1], $data_array[0], json_encode(['Cw_id' => $data_array[0], 'result' => 0]));  # 查询结果 0 查询失败 1不够卖 2购买
                            return false;
                        }
                        #返回正确是数据 获取宠物的数据
                        ...
                        try {
                            ...
                            if ($res) {
                                ...
                                $redis->hSet($data_array[1], $data_array[0], json_encode(['Cw_id' => $data_array[0], 'result' => 2]));  # 查询结果 0 查询失败 1不够卖 2购买

                            } else {
                                $redis = RedisPool::defer("redis");
                                $redis->hSet($data_array[1], $data_array[0], json_encode(['Cw_id' => $data_array[0], 'result' => 1]));  # 查询结果 0 查询失败 1不够卖 2购买
                                var_dump("不存在");
                            }
